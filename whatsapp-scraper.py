...
class Bot:
    def __init__(self):
        # To automate the execution of a Python Selenium script using a cron job
        os.environ["DISPLAY"] = ":0"
        # Alternatively, specify the display in the cron job: 30 2 * * * export DISPLAY=:0 && /path/to/your_script.py
        # Set up logging
        ...
        # Use Headless Mode: Configure Selenium to run in headless mode, which doesn't require a display:
        self.chrome_options.add_argument("--headless")
        #set this from Profile Path in chrome://version 
        self.chrome_options.add_argument(
            "user-data-dir=/tmp/.org.chromium.Chromium.qqng94"
        )
        # download_dir = os.path.join(os.getcwd(), "WhatsApp_Status")
        self.download_dir = (
            "/home/atefe_hjn97/Documents/VScode/instagramBot/WhatsApp_Status"
        )
        os.makedirs(
    ...
    def download_status(self):
        ...
        try:
            # Click on "My Status"
            btn_status = self.waits.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, 'button[aria-label="Status"]')
                )
            )
            btn_status.click()
            self.logger.info('Clicked on "Status"')

            btn_mystatus = self.waits.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, 'div[aria-label="My status"]')
                )
            )
            btn_mystatus.click()
            ...
            while True:
                ...
                try:
                    ...
                    btn_pause = self.waits2.until(
                        EC.element_to_be_clickable(
                            (
                                By.XPATH,
                                '//*[@id="app"]/div/span[3]/div/div/div/span/div/div/div/div[2]/div[3]/div/div/button/button',
                            )
                        )
                    )
                    btn_pause.click()
                    ...
                    # Open menu options
                    btn_menu = self.waits2.until(
                        EC.element_to_be_clickable(
                            (By.CSS_SELECTOR, 'span[data-icon="menu"]')
                        )
                    )
                    btn_menu.click()
                    ...
                    try:
                        btn_download = self.waits2.until(
                            EC.element_to_be_clickable(
                                (By.CSS_SELECTOR, 'div[aria-label="Download"]')
                            )
                        )
                        btn_download.click()
                        ...
                        self.logger.warning(
                            f"No downloadable image/video for status {status_count}"
                        )

                    # Click "Next" to proceed to the next status
                    btn_next = self.waits2.until(
                        EC.element_to_be_clickable(
                            (
                                By.CSS_SELECTOR,
                                "#app > div > span:nth-child(4) > div > div > div > div.x10l6tqk.x13vifvy.x1tav4y9.x1ey2m1c.xv97iv4.xzuapc8.xqvfhly.xr1yuqi.x4ii5y1.x1ypdohk.x66m237.x14yjl9h.xudhj91.x18nykt9.xww2gxu.x1vx7kgm",
                            )
                        )
                    )
                    btn_next.click()
                    ...
                    self.logger.error(
                        f"Error while navigating or downloading status {status_count}: {e}"
                    )
